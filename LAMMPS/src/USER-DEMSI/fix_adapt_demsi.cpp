...
enum{DIAMETER,CHARGE,THICKNESS};

/* ---------------------------------------------------------------------- */

FixAdaptDEMSI::FixAdaptDEMSI(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg),
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"pair") == 0) {
      ...
      if (strcmp(arg[iarg+1],"diameter") == 0) {
        ...
        chgflag = 1;
      } else if (strcmp(arg[iarg+1],"thickness") == 0) {
        adapt[nadapt].aparam = THICKNESS;
	thickflag = 1;      
      } else error->all(FLERR,"Illegal fix adapt command");
...
void FixAdaptDEMSI::post_constructor()
{
  if (!resetflag) return;
...
void FixAdaptDEMSI::setup_pre_force(int /*vflag*/)
{
  change_settings();
}
...
void FixAdaptDEMSI::setup_pre_force_respa(int vflag, int ilevel)
{
...
void FixAdaptDEMSI::pre_force(int /*vflag*/)
{
  if (nevery == 0) return;
...
void FixAdaptDEMSI::pre_force_respa(int vflag, int ilevel, int)
{
  if (ilevel < nlevels_respa-1) return;
  pre_force(vflag);
}
...
void FixAdaptDEMSI::change_settings()
{
  int i,j;
  ...
  for (int m = 0; m < nadapt; m++) {
	...
	double *thickness = atom->mean_thickness;
        int *mask = atom->mask;
        int nlocal = atom->nlocal;
        ...
        if (mflag == 0) {
          ...
          for (i = 0; i < nall; i++)
            if (mask[i] & groupbit) {
              density = rmass[i] / (MY_PI*radius[i]*radius[i]*thickness[i]);
              radius[i] *= value;
              rmass[i] = density*MY_PI*radius[i]*radius[i]*thickness[i];
            }
        }
...
void FixAdaptDEMSI::restore_settings()
{
  for (int m = 0; m < nadapt; m++) {
    Adapt *ad = &adapt[m];
    if (ad->which == PAIR) {
...
void FixAdaptDEMSI::set_arrays(int i)
{
