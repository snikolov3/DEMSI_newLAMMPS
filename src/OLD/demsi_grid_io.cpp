...
void GridWrite::write_field(Kokkos::View<double**>* field, const int nc_id, const size_t iTime) {
  ...
  log->check(err == MPI_SUCCESS, "Problem gathering field: ", (std::string) mpiErrBuffer);
  ...
  if (partition->on_master_proc()) {
    ...
    log->check(err == NC_NOERR, "Problem getting field varid: ", fieldName, ": ", nc_strerror(err));
    ...
    log->check(err == NC_NOERR, "Problem writing field: ", nc_strerror(err));
...
void GridWrite::write(void) {
  ...
  if (active and outputAlarm.is_ringing()) {
	...
	log->check(err == NC_NOERR, "Problem defining Time dimension: ", nc_strerror(err));

	err = nc_def_dim(nc_id, "strLen", 64, &dimidStrLen);
	log->check(err == NC_NOERR, "Problem defining strLen dimension: ", nc_strerror(err));

	err = nc_def_dim(nc_id, "nx", grid->globalDimensions[0], &dimidnx);
	log->check(err == NC_NOERR, "Problem defining nx dimension: ", nc_strerror(err));

	err = nc_def_dim(nc_id, "ny", grid->globalDimensions[1], &dimidny);
	log->check(err == NC_NOERR, "Problem defining ny dimension: ", nc_strerror(err));

	err = nc_def_dim(nc_id, "TWO", 2, &dimidTWO);
	log->check(err == NC_NOERR, "Problem defining TWO dimension: ", nc_strerror(err));
	...
	log->check(err == NC_NOERR, "Problem defining resolution variable: ", nc_strerror(err));
	...
	for (int iFieldOut = 0; iFieldOut < outputFieldNames.size() ; iFieldOut++) {
	  for (int iField = 0; iField < grid->fieldsGridWrite.size() ; iField++) {
	    if (outputFieldNames[iFieldOut] == grid->fieldsGridWrite[iField]->label()) {
	      ...
	      log->check(err == NC_NOERR, "Problem defining field: ", fieldName, ": ", nc_strerror(err));
	...
	err = nc_enddef(nc_id);
	log->check(err == NC_NOERR, "Problem endding definition phase: ", nc_strerror(err));
	...
	log->check(err == NC_NOERR, "Problem writing time: ", nc_strerror(err));
	delete [] strOut;
	...
	log->check(err == NC_NOERR, "Problem writing resolution: ", nc_strerror(err));

      } else if (err == NC_EEXIST) {

	// open preexisting file
	err = nc_open(filenameOut.c_str(), NC_WRITE, &nc_id);
	log->check(err == NC_NOERR, "Problem creating output eulerian file: ", filenameOut, ", :", nc_strerror(err));
	...
	log->check(err == NC_NOERR, "Problem getting Time dimension: ", nc_strerror(err));

	int varidTime;
	err = nc_inq_varid(nc_id, "Time", &varidTime);
	log->check(err == NC_NOERR, "Problem getting time varid: ", nc_strerror(err));
	...
	log->abort("Failed to create grid output file.");
      }
    ...
    if (partition->on_master_proc()) {
      ...
      log->check(err == NC_NOERR, "Problem closing file");
