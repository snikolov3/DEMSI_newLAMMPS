...
FixStyle(adapt/demsi,FixAdaptDEMSI)

#else

#ifndef LMP_FIX_ADAPT_DEMSI_H
#define LMP_FIX_ADAPT_DEMSI_H

#include "fix.h"

namespace LAMMPS_NS {

class FixAdaptDEMSI : public Fix {
 public:
  int diamflag;        // 1 if atom diameters will vary, for AtomVecGranular
  int chgflag;
  int thickflag;

  FixAdaptDEMSI(class LAMMPS *, int, char **);
  ~FixAdaptDEMSI();
  int setmask();
